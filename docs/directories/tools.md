...
Cet importeur synchronise les 17 modèles générés depuis `template-sources/`. Les sources sont réparties entre `Header_Template`, `Footer_Template`, `SideBar_Template`, `Cards_Templates`, `Form` et `Array_Template`.
...
Voir aussi [le dossier template-sources](template-sources.md).
